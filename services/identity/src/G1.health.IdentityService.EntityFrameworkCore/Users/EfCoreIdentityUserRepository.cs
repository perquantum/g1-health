...
namespace G1.health.IdentityService.EntityFrameworkCore.Users;

public class EfCoreIdentityUserRepository : EfCoreRepository<IIdentityServiceDbContext, IdentityUser, Guid>, IdentityService.Users.IIdentityUserRepository
{
    public EfCoreIdentityUserRepository(IDbContextProvider<IIdentityServiceDbContext> dbContextProvider)
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var list = dbContext.UserTenantAssociations.Select(x => x.UserId).ToList();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using (DataFilter.Disable<IMultiTenant>())
        {
            return await (await GetDbSetAsync())
            .IncludeDetails(includeDetails)
            .Where(u => list.Contains(u.Id) && u.Logins.Any(login => login.LoginProvider == loginProvider && login.ProviderKey == providerKey))
            .OrderBy(x => x.Id)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using (DataFilter.Disable<IMultiTenant>())
        {
            return await (await GetDbSetAsync())
            .IncludeDetails(includeDetails)
            .Where(u => list.Contains(u.Id) && u.Claims.Any(c => c.ClaimType == claim.Type && c.ClaimValue == claim.Value))
            .ToListAsync(GetCancellationToken(cancellationToken));
        }
    }
    ...
    public virtual async Task<List<IdentityUser>> GetListByIdsAsync(IEnumerable<Guid> ids, bool includeDetails = false, CancellationToken cancellationToken = default)
    {
        ...
        using (DataFilter.Disable<IMultiTenant>())
        {
            return await (await GetDbSetAsync())
            .IncludeDetails(includeDetails)
            .Where(x => list.Contains(x.Id) && ids.Contains(x.Id))
            .ToListAsync(GetCancellationToken(cancellationToken));
        }
    }
    ...
    public async Task<IList<string>> GetRoleNamesAsync(IdentityUser user, CancellationToken cancellationToken = default(CancellationToken))
    {
        var dbContext = await GetDbContextAsync();
        using (DataFilter.Disable<IMultiTenant>())
        {
            var roleIds = dbContext.Set<IdentityUserRole>().Where(x => x.UserId == user.Id).Select(x => x.RoleId).ToList();
            var result = dbContext.Roles.Where(x => roleIds.Contains(x.Id)).Select(x => x.Name).ToList();
            return result;
        }
    }

    public async Task<IdentityUser> FindByIdAsync(Guid id, CancellationToken cancellationToken = default(CancellationToken))
    {
        var dbContext = await GetDbContextAsync();
        var list = dbContext.UserTenantAssociations.Select(x => x.UserId).ToList();
    ...
    public override async Task<List<IdentityUser>> GetListAsync(bool includeDetails = false, CancellationToken cancellationToken = default(CancellationToken))
    {
        var dbContext = await GetDbContextAsync();
        var list = dbContext.UserTenantAssociations.Select(x => x.UserId).ToList();
        ...
        using (DataFilter.Disable<IMultiTenant>())
        {
            result = await (await GetDbSetAsync())
                .IncludeDetails(includeDetails)
                .Where(x => list.Contains(x.Id)).ToListAsync(GetCancellationToken(cancellationToken));
        }
        return result;
    }
    ...
    public override async Task<long> GetCountAsync(CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        using (DataFilter.Disable<IMultiTenant>())
        {
            count = await (await GetDbSetAsync()).Where(x => list.Contains(x.Id)).LongCountAsync(GetCancellationToken(cancellationToken));
        }
        return count;
    }

    public override async Task<List<IdentityUser>> GetPagedListAsync(int skipCount, int maxResultCount, string sorting, bool includeDetails = false, CancellationToken cancellationToken = default(CancellationToken))
    {
        var dbContext = await GetDbContextAsync();
        var list = dbContext.UserTenantAssociations.Select(x => x.UserId).ToList();
        ...
        using (DataFilter.Disable<IMultiTenant>())
        {
            ...
            .IncludeDetails(includeDetails)
            .Where(x => list.Contains(x.Id))
            .OrderBy(sorting.IsNullOrWhiteSpace() ? nameof(IdentityUser.Name) : sorting)
            .PageBy(skipCount, maxResultCount)
            .ToListAsync(GetCancellationToken(cancellationToken));
        }
    }

    public override async Task<IdentityUser> GetAsync(Guid id, bool includeDetails = true, CancellationToken cancellationToken = default(CancellationToken))
    {
    ...
    public override async Task<IdentityUser?> FindAsync(Guid id, bool includeDetails = true, CancellationToken cancellationToken = default(CancellationToken))
    {
        var dbContext = await GetDbContextAsync();
        var list = dbContext.UserTenantAssociations.Select(x => x.UserId).ToList();
    ...
    public override async Task<IdentityUser> InsertAsync(IdentityUser entity, bool autoSave = false, CancellationToken cancellationToken = default(CancellationToken)) { return null; }

    public override async Task InsertManyAsync(IEnumerable<IdentityUser> entities, bool autoSave = false, CancellationToken cancellationToken = default(CancellationToken)) { }

    public override async Task<IdentityUser> UpdateAsync(IdentityUser entity, bool autoSave = false, CancellationToken cancellationToken = default(CancellationToken)) { return null; }

    public override async Task UpdateManyAsync(IEnumerable<IdentityUser> entities, bool autoSave = false, CancellationToken cancellationToken = default(CancellationToken)) { }

    public override async Task DeleteAsync(IdentityUser entity, bool autoSave = false, CancellationToken cancellationToken = default(CancellationToken)) { }

    public override async Task DeleteManyAsync(IEnumerable<IdentityUser> entities, bool autoSave = false, CancellationToken cancellationToken = default(CancellationToken)) { }
