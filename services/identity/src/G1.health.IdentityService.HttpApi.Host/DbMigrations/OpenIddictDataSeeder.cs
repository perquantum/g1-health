...
public class OpenIddictDataSeeder : IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateApiScopesAsync()
    {
        ...
        await CreateScopesAsync("ClinicService");
        await CreateScopesAsync("AppointmentService");
        await CreateScopesAsync("FormsService");
    }

    private async Task CreateWebGatewaySwaggerClientsAsync()
    {
        await CreateSwaggerClientAsync("WebGateway", new[] { "AccountService", "IdentityService", "AdministrationService", "SaasService", "ProductService", "ClinicService", "AppointmentService", "FormsService" });
    }
    ...
    private async Task CreateSwaggerClientAsync(string name, string[]? scopes = null)
    {
        ...
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            ...
            var clinicServiceRootUrl = _configuration[$"OpenIddict:Resources:ClinicService:RootUrl"]?.TrimEnd('/');
            var AppointmentServiceRootUrl = _configuration[$"OpenIddict:Resources:AppointmentService:RootUrl"]?.TrimEnd('/');
            var formsServiceRootUrl = _configuration[$"OpenIddict:Resources:FormsService:RootUrl"]?.TrimEnd('/');
            ...
            await CreateApplicationAsync(
                ...
                redirectUris: new List<string> {
                    ...
                    $"{clinicServiceRootUrl}/swagger/oauth2-redirect.html", // ClinicService redirect uri
                    $"{AppointmentServiceRootUrl}/swagger/oauth2-redirect.html", // AppointmentService redirect uri
                    $"{formsServiceRootUrl}/swagger/oauth2-redirect.html", // FormsService redirect uri
                }
            );
        }
    }
    ...
    private async Task CreateClientsAsync()
    {
        ...
        await CreateApplicationAsync(
            ...
            scopes: commonScopes.Union(new[] {"AccountService", "IdentityService", "AdministrationService", "SaasService", "ProductService", "ClinicService", "AppointmentService", "FormsService" }).ToList(),
        ...
        await CreateApplicationAsync(
            ...
            scopes: commonScopes.Union(new[] { "AccountService", "AdministrationService", "ProductService" , "ClinicService" , "AppointmentService", "FormsService" }).ToList(),
