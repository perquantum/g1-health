    ...
    public async Task SeedAsync(Guid? tenantId = null)
    {
        using (_currentTenant.Change(tenantId))
        {
            using (var uow = _unitOfWorkManager.Begin(requiresNew: true, isTransactional: true))
            {
                ...
                var excludedPermissions = AdministrationServiceDataSeederConsts.ExcludedPermissions;
                var permissionNames = (await _permissionDefinitionManager
                    .GetPermissionsAsync())
                    .Where(p => p.MultiTenancySide.HasFlag(multiTenancySide))
                    .Where(p => !excludedPermissions.Contains(p.Name))
                ...
                await _permissionDataSeeder.SeedAsync(
                    ...
                    AdministrationServiceDataSeederConsts.ProviderKey,
                    permissionNames,
                    tenantId
                );
